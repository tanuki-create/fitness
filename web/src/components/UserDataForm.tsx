...
export function UserDataForm({ onNext }: { onNext: (data: UserData) => void }) {
  ...
  const UserProfileForm = ({ readOnly }: { readOnly: boolean }) => (
    <div className="space-y-6 py-4">
      ...
      <fieldset className="space-y-4">
        <legend className="text-base font-medium text-foreground">身体データ（任意）</legend>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
             <div className="space-y-2">
                <Label htmlFor="skeletalMuscleMass">筋肉量 (kg)</Label>
                <Input id="skeletalMuscleMass" name="skeletalMuscleMass" type="number" value={userData.skeletalMuscleMass} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="bodyFatPercentage">体脂肪率 (%)</Label>
                <Input id="bodyFatPercentage" name="bodyFatPercentage" type="number" value={userData.bodyFatPercentage} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="bodyFatMass">体脂肪量 (kg)</Label>
                <Input id="bodyFatMass" name="bodyFatMass" type="number" value={userData.bodyFatMass} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="smi">SMI (骨格筋指数)</Label>
                <Input id="smi" name="smi" type="number" value={userData.smi} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="bmr">基礎代謝量 (kcal)</Label>
                <Input id="bmr" name="bmr" type="number" value={userData.bmr} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="visceralFatLevel">内臓脂肪レベル</Label>
                <Input id="visceralFatLevel" name="visceralFatLevel" type="number" value={userData.visceralFatLevel} onChange={handleChange} readOnly={readOnly} />
            </div>
            <div className="space-y-2">
                <Label htmlFor="inbodyScore">InBodyスコア</Label>
                <Input id="inbodyScore" name="inbodyScore" type="number" value={userData.inbodyScore} onChange={handleChange} readOnly={readOnly} />
            </div>
        </div>
      </fieldset>
    </div>
